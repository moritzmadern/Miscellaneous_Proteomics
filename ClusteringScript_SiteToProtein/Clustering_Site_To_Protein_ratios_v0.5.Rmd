...
filepath_sites = "Results/PhosphoSites_SiteToProtein_TcellActivation.txt"
...
## Specify pearsons correlation coefficient threshold. Features with lower correlation to their respective cluster center will be kicked out of their clusters and instead be assigned to cluster 0
corr_threshold = 0.85
...
### 4) Specify parameters specific to ORA enrichment/overrepresentation analysis. 
...
## Create folder geneLists storing genes name info per k-means cluster
if (!"Lists_kMeans" %in% list.files("./Results")){
    dir.create("./Results/Lists_kMeans")
}


## Create folder Fig_kmeans storing images of k-means cluster
if (!"Fig_kmeans" %in% list.files("./Results")){
    dir.create("./Results/Fig_kmeans")
}


## Create folder geneLists storing genes name info per wgcna cluster
if (!"Lists_wgcna" %in% list.files("./Results")){
    dir.create("./Results/Lists_wgcna")
}

## Create folder Fig_wgcna storing images of wgcna cluster
if (!"Fig_wgcna" %in% list.files("./Results")){
    dir.create("./Results/Fig_wgcna")
}
...
df$entrezID <- mapIds(mouse, keys = df$Protein.names_first, keytype = "UNIPROT", column = "ENTREZID", multiVals="first")
...
df$FeatureCategory <- ifelse(df$unique_site_identifier %in% df_meta$unique_site_identifier, yes="DE_minimalChange", no = df$FeatureCategory)
...
```{r filter for highly-differentially abundant features based on maximum absolute log2 median FC, echo = FALSE, message = FALSE, warning = FALSE, fig.align="center", fig.path='fig_kmeansClustering/', dev=c('png', 'pdf')}
...
abline(v=max_abs_log2_FC_threshold, col="red", lty="dashed", lwd=2)
...
## Document in unfiltered table df which site features (rows) have been filtered out etc
df$FeatureCategory <- ifelse(df$unique_site_identifier %in% df_meta$unique_site_identifier, yes="highly_DE", no = df$FeatureCategory)
...
Note: Features that don't fit too well to their respective cluster center (as determined via pearosn correlation coefficient) will be assigned to a new cluster labelled 0.
...
v_df_Cluster <- setNames(character(nrow(df)), nm=df$unique_site_identifier)
v_df_Cluster[as.character(df_meta$unique_site_identifier)] <- df_meta$kmeans_cluster
...
Note: This clustering procedure is at core a hyrarchical clustering based on an adjacency matrix, implementing an arbitrary cutoff for the tree.
...
## Document in unfiltered table df which site features (rows) fall in which cluster
v_df_Cluster <- setNames(character(nrow(df)), nm=df$unique_site_identifier)
v_df_Cluster[as.character(df_meta$unique_site_identifier)] <- df_meta$wgcna_cluster
...
top_annotation = HeatmapAnnotation(Timepoint = groups, col = list(Timepoint = colors_groups))
m_heatmap <- df[,grepl(names(df), pattern=pattern_quant)] %>% as.matrix(.) %>% log2(.)
rownames(m_heatmap) = NULL
colnames(m_heatmap) <- samplenames
...
pdf("Results/Heatmap_SiteToProtein_ClusteringResults.pdf", height=6.5, width=6)
c_heatmap
dev.off()
